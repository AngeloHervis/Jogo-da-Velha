const boardRegions = document.querySelectorAll('#gameBoard .cell');
let vBoard = [];
let turnPlayer = '';
...
function handleWin(regions) {
    ...
    // Desabilita todas as regiões após a vitória
    boardRegions.forEach(function(element) {
        element.removeEventListener('click', handleBoardClick);
    });
}
...
function handleBoardClick(ev) {
    ...
    if (vBoard[row][column] !== '') {
        // Se a célula já estiver preenchida, retorna sem fazer nada
        return;
    }
